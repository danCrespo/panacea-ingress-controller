...
func (rt *routingTable) UpdateFromIngresses(_ingresses []*networkingv1.Ingress, ingressClass string) {
	...
	domain, err := kubeutils.NewKubeutils().GetClusterDomain()
		...
		for _, rule := range ingress.Spec.Rules {
			...
			for _, path := range rule.HTTP.Paths {
				...
				if path.Backend.Resource != nil {
					...
					clusterResource, err := kubeutils.NewKubeutils().GetResource(ingress.Namespace, resource.Kind, resource.Name)
					if err != nil {
				...
				if path.Backend.Service.Port.Number != 0 {
					...
					svcPort, err := kubeutils.NewKubeutils().GetServicePortByName(ingress.Namespace, name, svc.Port.Name)
					if err != nil {
	...
	l.Info("Routing table updated", "data", rt.data)
}
...
func (rt *routingTable) GetRoutes(ingressKey string) []*Route {
	...
	if _, exists := rt.data[ingressKey]; !exists {
		return nil
	}
